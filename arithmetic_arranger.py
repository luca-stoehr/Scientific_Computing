def arithmetic_arranger(problems, solve=False):
  ...
  for problem in problems:
    ...
    check_syntax(problem, operator)
    (first_line[idx],second_line[idx]) = calc_separator(problem, operator)
    ...
    if operator:
      if solve:
        ...
        forth_line[idx] = "{}{}".format(" " * (block_length - len(forth_line[idx])), forth_line[idx])
      second_line = "{}{}{}".format("+"," " * (block_length - len(second_line[idx] - 1)), second_line[idx])
    else:
      if solve:
        forth_line[idx] = int(first_line[idx]) - int(second_line[idx])
        forth_line[idx] = "{}{}".format(" " * (block_length - len(forth_line[idx])), forth_line[idx])
    ...
    idx += 1
  arranged_problems_base = "\n".join(["    ".join(first_line), "    ".join(third_line), "    ".join(third_line)])
  if solve:
